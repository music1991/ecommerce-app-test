      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-xs text-slate-500 line-clamp-1">{row.original.description}</span>
          </div>
        </div>
      ),
    },
      ...
      cell: ({ getValue }) => {
        ...
        return (
          <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase ${
            stock <= 3 ? "bg-rose-100 text-rose-700" : "bg-emerald-100 text-emerald-700"
          }`}>
            {stock} unidades
          </span>
        );
    ...
    },
    {
      ...
      cell: ({ getValue }) => (
        <span className="font-black text-slate-900">
          {new Intl.NumberFormat("es-AR", { style: "currency", currency: "ARS" }).format(Number(getValue()))}
        </span>
      ),
    },
    {
