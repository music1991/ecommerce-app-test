...
import { Trash2, ShoppingBag, CreditCard, Store, XCircle } from "lucide-react";

import { useCartStore } from "../store/useCartStore";
import { useSalesStore } from "../../sales/store/useSalesStore";
import { useAuthStore } from "../../auth/store/authStore";
import { SaleMercadoPagoRedirectModal } from "../../admin/components/SaleMercadoPagoRedirectModal";
...
export const Cart = () => {
  ...
  const handleContinueMercadoPago = () => {
    if (!saleId) return;
/*     setIsRedirectModalOpen(false);
    navigate(`/sales/payment/${saleId}`); */
  };
  ...
  return (
    <>
        ...
        <div className="flex justify-between items-center mb-10">
          <div>
            ...
            <p className="text-slate-500">
              {currentActor.name} • ID Venta: #{saleId}
            </p>
          </div>

          <button
            onClick={handleCancel}
            className="text-slate-400 hover:text-red-500 flex items-center gap-2"
          >
            <XCircle size={20} />
            Cancelar
          </button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          <div className="lg:col-span-2 space-y-4">
            {cartItems.map((item) => (
              <div
                key={item.id}
                className="flex items-center gap-4 p-4 bg-white border border-slate-100 rounded-3xl shadow-sm"
              >
                <img
                  src={item.images?.[0]}
            ...
            <div className="bg-slate-900 text-white p-8 rounded-[2.5rem] sticky top-24">
              ...
              <div className="space-y-3">
                <button
                  ...
                  className="w-full bg-blue-600 hover:bg-blue-700 py-4 rounded-2xl font-black flex items-center justify-center gap-2"
                >
                  <CreditCard size={20} />
                  Comprar Online
                </button>
                ...
                <button
                  ...
                  className="w-full bg-white/10 hover:bg-white/20 py-4 rounded-2xl font-bold flex items-center justify-center gap-2"
                >
                  <Store size={18} />
                  Finalizar en Local
                </button>
              ...
              {sale && (
                <div className="mt-6 pt-6 border-t border-slate-800 text-xs text-slate-400">
                  <p>
                    Estado:{" "}
                    <span className="text-white uppercase font-black">{sale.status}</span>
                  </p>
      ...
      <SaleMercadoPagoRedirectModal
        isOpen={isRedirectModalOpen}
        onClose={() => setIsRedirectModalOpen(false)}
        total={cartTotal}
        customerName={currentActor.name}
        employeeName="Compra Online"
        onContinue={handleContinueMercadoPago}
      />
    </>
  );
};